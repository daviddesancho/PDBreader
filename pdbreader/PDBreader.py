...
class Protein:
    ...
    def __init__(self,filename=None):
        print "\n Generating class instance of Protein \n"
    	if filename:
            ...
            print [self.resid[x]['type'] for x in self.reslist]
    	    self.nres = len(self.reslist)
            print "\n %g residues read"%self.nres
    
    def read_coords(self):
        """ Read protein coordinates from PDB file """

        print " ...reading coodinates from file"
    ...
    def find_disulf(self, dcut=2.2):
        """ Find disulfides in protein """
        self.disulf = {}
        dcut2 = dcut**2
        print "\n ...calculating disulfide bonds based on distance "
        cys = [x for x in self.resid if self.resid[x]['type'] == 'CYS']
        for (i,j) in itertools.combinations(cys,2):
            SGi = [x for x in self.resid[i]['coords'] if x[0] == 'SG'][0]
            SGj = [x for x in self.resid[j]['coords'] if x[0] == 'SG'][0]
            d2 = dist2(SGi[1], SGj[1])
            if d2 < dcut2:
                print i,j,math.sqrt(d2)
                self.disulf[(i,j)] = {}
                self.disulf[(i,j)]['dist'] = math.sqrt(d2)
